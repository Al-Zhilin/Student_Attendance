uint8_t checkTableWeek() {            //функция проверки и достроения недель в Google Sheet
  ...
  cell_data = answer.getSub(r_count, "\"");            //содержит данные из ячейки с краткой информацией. Начинаем доставать нужные нам величины

  for (byte iter = 0; iter < ans.count("/"); iter++) {        //проходим по всем величинам
    Text get_cell = ans.getSub(iter, "/");
    if (!iter) {
      if (get_cell == "Числитель" || get_cell == "числитель") last_parity = true;
      else  last_parity = false;
    }
